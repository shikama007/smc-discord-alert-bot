...
# STEP 6 — DISPLACEMENT
DISPLACEMENT_LOOKAHEAD = 6       # closed candles after the sweep
DISPLACEMENT_AVG_LOOKBACK = 10   # candles used for baseline body/range
DISPLACEMENT_MIN_BODY_RATIO = 1.35
DISPLACEMENT_MIN_RANGE_RATIO = 1.20
DISPLACEMENT_MIN_BODY_PCT = 0.15
DISPLACEMENT_MIN_CLOSE_LOCATION = 0.65
DISPLACEMENT_MIN_SCORE = 4
...
def detect_liquidity_sweeps(
    ...
    - POI interaction is time-consistent: the POI must already exist
      strictly before the sweep candle. Same-candle POIs are rejected.
    ...
    for i in range(start, len(closed)):
        ...
        def find_historical_poi(direction):
            ...
            for poi in all_pois:
                ...
                # the POI must exist STRICTLY BEFORE the sweep candle.
                # A POI created on the same candle as the sweep cannot be
                # used to confirm that sweep, because the POI itself was
                # only knowable after that candle closed.
        ...
        for target in unique_highs:
            ...
            if high > level and close < level:
                ...
                sweeps.append({
                    ...
                    "poi_same_candle_rejected": False,
                    "poi": historical_poi
                })
...
def _displacement_metrics(candle, avg_body, avg_range):
    """Return normalized displacement metrics for one closed candle."""
    ...
    range_ratio = candle_range / avg_range if avg_range > 0 else 0.0
    body_pct = (body / open_price) * 100 if open_price > 0 else 0.0
...
def _candle_creates_fvg(closed, index, direction):
    """Check whether the candidate displacement candle creates a 3-candle FVG."""
    if index < 2 or index >= len(closed):
        return False

    c1 = closed.iloc[index - 2]
    c3 = closed.iloc[index]
...
def detect_displacement(df, sweep, bias):
    """
    Detect the first strong directional displacement after a confirmed sweep.

    Rules intentionally use only fully closed candles and require the
    displacement candle to occur AFTER the sweep candle.

    A candidate is confirmed when it has:
      - strong body relative to recent average body,
      - expanded range relative to recent average range,
      - meaningful body size as a % of price,
      - close near the directional extreme,
      - plus either an FVG or an even stronger body expansion.

    This is a mechanical detector, not a probability/entry prediction.
    """
    if sweep is None:
        return {
            "status": "NO SWEEP",
            ...
            "direction": None,
            "candle": None,
            "reason": "No confirmed sweep to evaluate."
        }
    ...
    if closed.empty:
        return {
            ...
            "direction": None,
            "candle": None,
            "reason": "No fully closed candles available."
        }
    ...
    if direction not in ("BULLISH", "BEARISH"):
        return {
            "status": "INVALID SWEEP DIRECTION",
            "confirmed": False,
            "direction": direction,
            "candle": None,
            "reason": "Sweep direction is not bullish or bearish."
        }

    if sweep_index < 0 or sweep_index >= len(closed) - 1:
        return {
            "status": "WAITING FOR POST-SWEEP CANDLE",
            "confirmed": False,
            "direction": direction,
            "candle": None,
            "reason": "No closed candle exists after the sweep yet."
        }

    start = sweep_index + 1
    end = min(len(closed), start + DISPLACEMENT_LOOKAHEAD)
    ...
    for i in range(start, end):
        ...
        candle_direction = (
            ...
            else "BEARISH" if close < open_price
            else "NEUTRAL"
        )
        ...
        base_start = max(0, i - DISPLACEMENT_AVG_LOOKBACK)
        baseline = closed.iloc[base_start:i]

        if len(baseline) < 5:
            continue

        avg_body = float((baseline["close"] - baseline["open"]).abs().mean())
        avg_range = float((baseline["high"] - baseline["low"]).mean())

        metrics = _displacement_metrics(candle, avg_body, avg_range)
        fvg_created = _candle_creates_fvg(closed, i, direction)

        score = 0
        reasons = []

        if metrics["body_ratio"] >= DISPLACEMENT_MIN_BODY_RATIO:
            score += 1
            reasons.append("STRONG BODY")

        if metrics["range_ratio"] >= DISPLACEMENT_MIN_RANGE_RATIO:
            score += 1
            reasons.append("EXPANDED RANGE")

        if metrics["body_pct"] >= DISPLACEMENT_MIN_BODY_PCT:
            score += 1
            reasons.append("MEANINGFUL BODY")

        if metrics["close_location"] >= DISPLACEMENT_MIN_CLOSE_LOCATION:
            score += 1
            reasons.append("CLOSE NEAR EXTREME")

        if fvg_created:
            score += 1
            reasons.append("FVG CREATED")

        # A very large body can qualify for the final condition even when
        # a clean 3-candle FVG is not produced.
        very_strong_body = metrics["body_ratio"] >= 1.70

        confirmed = (
            score >= DISPLACEMENT_MIN_SCORE
            and (
                fvg_created
                or very_strong_body
            )
    ...
    confirmed_candidates = [c for c in candidates if c["confirmed"]]

    if confirmed_candidates:
        # Earliest confirmed displacement after the sweep is the one we use.
        chosen = confirmed_candidates[0]
        status = (
            "DISPLACEMENT CONFIRMED"
            if direction == bias
            else "COUNTER-BIAS DISPLACEMENT"
        )
        return {
            "status": status,
            "confirmed": True,
            "direction": direction,
            "bias_aligned": direction == bias,
            "candle": chosen,
            "candidates_checked": len(candidates),
            "reason": "; ".join(chosen["reasons"]),
        }

    # Keep the strongest directional candidate for diagnostics.
    strongest = None
    if candidates:
        strongest = sorted(
            candidates,
    ...
    return {
        "status": "NO CONFIRMED DISPLACEMENT",
        "confirmed": False,
        "direction": direction,
        "bias_aligned": direction == bias,
        "candle": strongest,
        "candidates_checked": len(candidates),
        "reason": (
            "No candle met all displacement rules."
            if strongest is None
            else "Strongest candidate: " + ", ".join(strongest["reasons"])
        )
    }


def get_step6_status(df, step5, bias):
    """Evaluate Step 6 from the latest active Step 5 sweep."""
    ...
    # If the sweep already has a later event in the same direction, we still
    # evaluate candles after the sweep. Step 6 remains independent from BOS.
    displacement = detect_displacement(df, sweep, bias)

    return {
        "status": displacement["status"],
        "confirmed": displacement["confirmed"],
...
# =========================================================
# STEP 8 — RETEST / RECLAIM
# =========================================================
# Strict sequence:
# Step 5  -> Liquidity Sweep
# Step 6  -> Displacement
# Step 7  -> Internal CHOCH/BOS
# Step 8  -> Retest of the broken structure level
#
# Rules:
# - Step 8 cannot run without a confirmed Step 7 break.
# - Only fully closed candles are evaluated.
# - The retest must happen AFTER the BOS candle.
# - Bullish: candle must revisit/touch the broken level and close back ABOVE it.
# - Bearish: candle must revisit/touch the broken level and close back BELOW it.
# - Wick-only contact is not enough; the candle must reclaim/hold the level
#   on close.
# - The original BOS candle is never counted as its own retest.
# =========================================================

STEP8_LOOKAHEAD = 8
STEP8_RETEST_TOLERANCE_PCT = 0.15
STEP8_MIN_REJECTION_RATIO = 0.25


def _retest_metrics(candle, level_price, direction):
    """Calculate whether one closed candle meaningfully retests a broken level."""
    open_price = float(candle["open"])
    high = float(candle["high"])
    low = float(candle["low"])
    close = float(candle["close"])
    ...
    tolerance = level_price * (STEP8_RETEST_TOLERANCE_PCT / 100.0)

    if direction == "BULLISH":
        # Price must come back down to the broken level area.
        touched = low <= level_price + tolerance and high >= level_price - tolerance
        reclaimed = close > level_price

        # Lower wick/rejection below the level is useful evidence that
        # price tested the level and was rejected back upward.
        lower_wick = max(0.0, min(open_price, close) - low)
        rejection_ratio = lower_wick / candle_range if candle_range > 0 else 0.0

    elif direction == "BEARISH":
        # Price must come back up to the broken level area.
        touched = high >= level_price - tolerance and low <= level_price + tolerance
        reclaimed = close < level_price

        # Upper wick/rejection above the level is useful evidence that
        # price tested the level and was rejected back downward.
        upper_wick = max(0.0, high - max(open_price, close))
        rejection_ratio = upper_wick / candle_range if candle_range > 0 else 0.0
...
def detect_retest(df, structure_break):
    """
    Confirm the first valid retest after Step 7 structure break.

    A valid retest requires:
      1. Step 7 is confirmed.
      2. A closed candle after the BOS candle reaches the broken level area.
      3. The same candle closes back on the correct side of the broken level.

    This prevents a mere wick touch or a continuation candle far away from
    the level from being labelled as a retest.
    """
    if not structure_break or not structure_break.get("confirmed"):
        return {
            "status": "WAITING FOR CONFIRMED STEP 7",
            "confirmed": False,
            "retest": None,
        }
    ...
    level_price = structure_break.get("broken_level")
    break_time = structure_break.get("break_time")

    if direction not in ("BULLISH", "BEARISH"):
        return {
            "status": "INVALID STEP 7 DIRECTION",
            "confirmed": False,
            "retest": None,
        }

    if level_price is None or break_time is None:
        return {
            "status": "INCOMPLETE STEP 7 DATA",
            "confirmed": False,
            "retest": None,
        }

    matches = closed.index[closed["time"] == break_time].tolist()
    if not matches:
        return {
            "status": "BREAK CANDLE NOT FOUND",
            "confirmed": False,
            "retest": None,
        }

    break_index = int(matches[-1])
    start = break_index + 1
    end = min(len(closed), start + STEP8_LOOKAHEAD)
    ...
    if start >= end:
        return {
            ...
            "confirmed": False,
            "retest": None,
            "broken_level": float(level_price),
            "reason": "No closed candle after Step 7 break is available yet.",
        }
    ...
    for i in range(start, end):
        candle = closed.iloc[i]
        metrics = _retest_metrics(candle, float(level_price), direction)

        # A close clearly through the broken level in the wrong direction
        # means the break has not held. Keep this diagnostic separate from
        # a confirmed retest.
        if direction == "BULLISH":
            if metrics["close"] < float(level_price) and not metrics["touched"]:
                invalidation = {
                    "index": i,
                    "time": candle["time"],
                    "close": metrics["close"],
                    "reason": "CLOSE BELOW BROKEN LEVEL",
                }
        elif direction == "BEARISH":
            if metrics["close"] > float(level_price) and not metrics["touched"]:
                invalidation = {
                    ...
                    "time": candle["time"],
                    "close": metrics["close"],
                    "reason": "CLOSE ABOVE BROKEN LEVEL",
                }

        if metrics["touched"] and metrics["reclaimed"]:
            rejection_confirmed = (
                metrics["rejection_ratio"] >= STEP8_MIN_REJECTION_RATIO
                or (
                    direction == "BULLISH"
            ...
            retest = {
                ...
                "confirmation": "CLOSE HOLDS ABOVE LEVEL" if direction == "BULLISH" else "CLOSE HOLDS BELOW LEVEL",
            }

            return {
                "status": "STEP 8 — RETEST CONFIRMED",
                "confirmed": True,
                "direction": direction,
                "broken_level": float(level_price),
                "break_time": break_time,
                "retest": retest,
                "reason": retest["confirmation"],
            }
    ...
    result = {
        ...
        "retest": None,
        "broken_level": float(level_price),
        "reason": "No valid retest has closed back on the correct side of the broken level yet.",
    }
...
def get_step8_status(df, step7):
    """Main Step 8 wrapper. Step 8 only starts after Step 7 confirmation."""
    if not step7 or not step7.get("confirmed"):
        return {
            "status": "WAITING FOR CONFIRMED STEP 7",
            "confirmed": False,
            "retest": None,
    ...
    structure_break = step7.get("structure_break")
    return detect_retest(df, structure_break)
...
print()
print("🚀 ICT/SMC ENGINE — STEP 6 DISPLACEMENT")
print("============================================")
...
step7 = get_step7_status(df, step5, step6)

# =========================================================
# STEP 8 — RETEST / RECLAIM
# =========================================================

step8 = get_step8_status(df, step7)
...
print()
print("📈 STEP 6 — DISPLACEMENT")
print(f"Status: {step6['status']}")

if step6.get("displacement"):
    disp = step6["displacement"]
    print(f"Direction: {disp.get('direction')}")
    print(f"Confirmed: {'YES' if disp.get('confirmed') else 'NO'}")
    if disp.get("candle"):
        dc = disp["candle"]
        print(f"Candle Time: {dc['time']}")
        print(f"Open: ${dc['open']:.4f}")
        print(f"High: ${dc['high']:.4f}")
        print(f"Low: ${dc['low']:.4f}")
        print(f"Close: ${dc['close']:.4f}")
        print(f"Body Ratio: {dc['body_ratio']:.2f}x")
        print(f"Range Ratio: {dc['range_ratio']:.2f}x")
        print(f"Body: {dc['body_pct']:.3f}%")
        print(f"Close Location: {dc['close_location']:.2f}")
        print(f"FVG Created: {'YES' if dc['fvg_created'] else 'NO'}")
        print(f"Score: {dc['score']}")
        print(f"Reasons: {', '.join(dc['reasons'])}")
    print(f"Reason: {disp.get('reason', 'None')}")

print()
print("🧠 STEP 7 — POST-DISPLACEMENT CHOCH / BOS")
print(f"Status: {step7['status']}")
print(f"Confirmed: {'YES' if step7['confirmed'] else 'NO'}")
if step7.get("structure_break"):
    sb = step7["structure_break"]
    if sb.get("level"):
        print(f"Broken Internal Level: ${sb['level']['price']:.4f}")
    if sb.get("candle"):
        c = sb["candle"]
        print(f"Break Direction: {c['direction']}")
        print(f"Event: {c['event']}")
        print(f"Break Candle: {c['time']}")
        print(f"Break Close: ${c['close']:.4f}")
        print(f"Break Method: {c['break_method']}")
    else:
        print(f"Reason: {sb.get('reason', 'None')}")

print()
print("🔁 STEP 8 — RETEST / RECLAIM")
print(f"Status: {step8['status']}")
print(f"Confirmed: {'YES' if step8['confirmed'] else 'NO'}")
if step8.get("broken_level") is not None:
    print(f"Broken Level: ${step8['broken_level']:.4f}")
if step8.get("retest"):
    rt = step8["retest"]
    print(f"Direction: {rt['direction']}")
    print(f"Retest Candle: {rt['time']}")
    print(f"Retest High: ${rt['high']:.4f}")
    print(f"Retest Low: ${rt['low']:.4f}")
    print(f"Retest Close: ${rt['close']:.4f}")
    print(f"Rejection Ratio: {rt['rejection_ratio']:.2f}")
    print(f"Confirmation: {rt['confirmation']}")
else:
    print(f"Reason: {step8.get('reason', 'None')}")
...
message += (
    ...
    f"Bias Alignment: **{premium_discount['alignment']}**\n"
)
...
if step5["latest"]:
    ...
    message += (
        ...
        f"Swept Level: `${sweep['level']:.4f}`\n"
        f"Liquidity Status: **SWEPT / NOT IN ACTIVE LIST**\n"
...
if step6.get("displacement"):
    ...
    if disp.get("candle"):
        dc = disp["candle"]
        message += (
            f"Direction: **{disp.get('direction')}**\n"
            f"Confirmed: **{'YES' if disp.get('confirmed') else 'NO'}**\n"
            f"Candle Time: `{dc['time']}`\n"
            f"Open: `${dc['open']:.4f}` | High: `${dc['high']:.4f}`\n"
            f"Low: `${dc['low']:.4f}` | Close: `${dc['close']:.4f}`\n"
            f"Body Ratio: `{dc['body_ratio']:.2f}x`\n"
            f"Range Ratio: `{dc['range_ratio']:.2f}x`\n"
            f"Body: `{dc['body_pct']:.3f}%`\n"
            f"Close Location: `{dc['close_location']:.2f}`\n"
            f"FVG Created: **{'YES' if dc['fvg_created'] else 'NO'}**\n"
            f"Score: `{dc['score']}`\n"
            f"Reasons: `{', '.join(dc['reasons'])}`\n"
        )
    else:
        message += f"Reason: `{disp.get('reason', 'None')}`\n"
else:
    message += "No displacement evaluation available.\n"

message += (
    "\n🧠 **STEP 7 — POST-DISPLACEMENT CHOCH / BOS**\n"
    f"Status: **{step7['status']}**\n"
    f"Confirmed: **{'YES' if step7['confirmed'] else 'NO'}**\n"
)

if step7.get("structure_break"):
    sb = step7["structure_break"]
    if sb.get("level"):
        message += f"Broken Internal Level: `${sb['level']['price']:.4f}`\n"
    if sb.get("candle"):
        c = sb["candle"]
        message += (
            f"Direction: **{c['direction']}**\n"
            f"Event: **{c['event']}**\n"
            f"Break Candle: `{c['time']}`\n"
            f"Break Close: `${c['close']:.4f}`\n"
            f"Break Method: `{c['break_method']}`\n"
        )
    else:
        message += f"Reason: `{sb.get('reason', 'None')}`\n"

message += (
    "\n🔁 **STEP 8 — RETEST / RECLAIM**\n"
    f"Status: **{step8['status']}**\n"
    f"Confirmed: **{'YES' if step8['confirmed'] else 'NO'}**\n"
)

if step8.get("broken_level") is not None:
    message += f"Broken Level: `${step8['broken_level']:.4f}`\n"

if step8.get("retest"):
    rt = step8["retest"]
    message += (
        f"Direction: **{rt['direction']}**\n"
        f"Retest Candle: `{rt['time']}`\n"
        f"Retest High: `${rt['high']:.4f}` | Low: `${rt['low']:.4f}`\n"
        f"Retest Close: `${rt['close']:.4f}`\n"
        f"Rejection Ratio: `{rt['rejection_ratio']:.2f}`\n"
        f"Confirmation: `{rt['confirmation']}`\n"
    )
else:
    message += f"Reason: `{step8.get('reason', 'None')}`\n"
...
message += (
    ...
    "✅ Displacement (Step 6)\n"
    "✅ Post-Displacement CHOCH/BOS (Step 7)\n"
    "✅ Retest / Reclaim (Step 8)\n"
)

print()
print("📡 Sending Step 7 result to Discord...")
send_discord_message(message)

print()
print("✅ STEP 8 RETEST TEST COMPLETE!")
