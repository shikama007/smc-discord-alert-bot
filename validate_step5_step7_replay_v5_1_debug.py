...
def fetch():
    # Use the exact MEXC Futures endpoint/response format already used by the
    # production engine. The previous V5 version incorrectly used a query-style
    # endpoint with "limit=1000", which returned HTTP 404 in GitHub Actions.
    ...
    if len(df) < WARMUP + 50:
        raise RuntimeError(
            f"Not enough candles returned: {len(df)}. "
            f"Need at least {WARMUP + 50}."
        )

    print(f"\U0001f4e1 Downloaded {len(df)} MEXC Futures candles")
    return df
...
def main():
    ...
    for n in range(start, end):
        ...
        try:
            # Current production engine requires bias as the second argument.
            # Keep replay aligned with the production function signature.
            bias = None

            # Prefer the production helper if present.
            if hasattr(engine, "determine_initial_bias"):
                try:
                    bias = engine.determine_initial_bias(snap)
                except Exception:
                    bias = None

            # Some engine versions expose the current structure/bias helper.
            if bias is None and hasattr(engine, "get_current_structure"):
                try:
                    structure = engine.get_current_structure(snap)
                    if isinstance(structure, dict):
                        bias = structure.get("bias") or structure.get("Bias")
                except Exception:
                    bias = None

            if bias is None:
                raise RuntimeError("Could not determine production bias for replay snapshot")

            step5 = engine.get_step5_status(snap, bias)
            step6 = engine.get_step6_status(snap, step5)
        except Exception as e:
            engine_error_count += 1
            print(f"[DEBUG] snapshot {snap.iloc[-1]['time']}: engine error: {e}")
            continue

        if isinstance(step5, dict) and step5.get("latest"):
            step5_active_count += 1

        disp_debug = step6.get("displacement") if isinstance(step6, dict) else None
        if disp_debug and disp_debug.get("confirmed"):
            step6_confirmed_count += 1
            if first_step6 is None:
                first_step6 = {
                    "asof": str(snap.iloc[-1]["time"]),
                    "status": step6.get("status"),
                    "direction": disp_debug.get("direction"),
                    "candle": disp_debug.get("candle"),
                    "sweep": step5.get("latest") if isinstance(step5, dict) else None,
                }
                print("\n=== FIRST STEP 6 CONFIRMATION ===")
                print(safe_json(first_step6))
                print("=================================\n")
    ...
    print("Engine errors:", engine_error_count)
    ...
    if first_step6 is None:
        print("WARNING: No Step 6 confirmation was captured in this replay.")
    else:
        print("First Step 6 confirmation was captured successfully.")
    print("=========================\n")
