...
def main():
    ...
    for end in range(WARMUP, len(df)):
        ...
        # every known case is evaluated on subsequent closed snapshots.
        for case in cases.values():
            ...
            # A case must remain trackable even when the current snapshot's
            # Step 6 object has moved on. Match the case to the production
            # Step 7 structure-break output when available.
            if (
                current_disp_time == case["displacement_time"]
                or (
                    step7
                    and step7.get("structure_break")
                    and str(
                        (step7["structure_break"].get("displacement") or {})
                        .get("time")
    ...
    cases_df.to_csv(
        "step7_v3_2_lifecycle_cases.csv",
        index=False,
    )
    snapshots_df.to_csv(
        "step7_v3_2_lifecycle_snapshots.csv",
        index=False,
    )
    ...
    print("\n=== STEP 7 V3.2 PRODUCTION-FETCH LIFECYCLE REPLAY ===")
    ...
    print("\nReports:")
    print(" - step7_v3_2_lifecycle_cases.csv")
    print(" - step7_v3_2_lifecycle_snapshots.csv")
